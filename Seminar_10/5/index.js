...
/**
 * EXPORT: GET /
 * Returns all universities with students, courses and enrollements
 */
application.get("/", async (request, response, next) => {
  try {
    ...
    // Now transform the data into required format.
    const result = universities.map((university) => {
      // Extract enrollements as pairs studentId–courseId
      const enrollements = [];
      ...
      return {
        ...
        courses: university.courses.map((c) => ({
          id: c.id,
